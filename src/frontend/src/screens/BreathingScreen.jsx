...
const BOX_PHASES = [
  { label: 'Inhale', duration: 4, color: '#4A90D9' },
  { label: 'Hold', duration: 4, color: '#7B5EA7' },
  { label: 'Exhale', duration: 4, color: '#27AE60' },
  { label: 'Hold', duration: 4, color: '#E67E22' },
];
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 24 }}>
      <svg width={160} height={160}>
        ...
        />
        <text x={80} y={76} textAnchor="middle" fontSize={28} fontWeight={700} fill="var(--color-text)">{seconds}</text>
        <text x={80} y={96} textAnchor="middle" fontSize={12} fill={phase.color} fontWeight={600}>{phase.label}</text>
      </svg>
      <div style={{ textAlign: 'center' }}>
        <p style={{ fontWeight: 600 }}>Box Breathing · Cycle {cycles + 1}</p>
        <p style={{ fontSize: '0.85rem' }}>Inhale 4 · Hold 4 · Exhale 4 · Hold 4</p>
      </div>
      {cycles >= 4 && (
...
const PHASES_478 = [
  { label: 'Inhale', duration: 4, color: '#4A90D9' },
  { label: 'Hold', duration: 7, color: '#7B5EA7' },
  { label: 'Exhale', duration: 8, color: '#27AE60' },
];
...
function Breathing478({ onDone }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 24 }}>
      ...
      <div style={{ textAlign: 'center' }}>
        <p style={{ fontWeight: 700, fontSize: '1.1rem', color: phase.color }}>{phase.label}</p>
        <p style={{ fontSize: '0.85rem' }}>4-7-8 Breathing · Cycle {cycles + 1}</p>
      </div>
      {cycles >= 4 && (
        <button className="btn btn--success" onClick={onDone}>✅ Done — I feel calmer</button>
      )}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      {done ? (
        <div style={{ textAlign: 'center' }}>
          ...
          <h3>Well done. You're here, you're present.</h3>
          <button className="btn btn--success" style={{ marginTop: 20 }} onClick={onDone}>✅ I feel grounded</button>
        </div>
      ) : (
        <>
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: 48, marginBottom: 8 }}>{current.emoji}</div>
            <h2 style={{ color: 'var(--color-primary)' }}>{current.count} — {current.sense}</h2>
            <p style={{ marginTop: 8, lineHeight: 1.6, fontSize: '0.95rem', color: 'var(--color-text)' }}>{current.prompt}</p>
          </div>
          ...
          <div style={{ display: 'flex', gap: 6, justifyContent: 'center' }}>
            {GROUNDING_STEPS.map((_, i) => (
              <div key={i} style={{ width: 8, height: 8, borderRadius: '50%', background: i === step ? 'var(--color-primary)' : 'var(--color-border)' }} />
            ))}
          </div>
        </>
      )}
...
function PMR({ onDone }) {
  ...
  if (done) {
    return (
      <div style={{ textAlign: 'center' }}>
        <div style={{ fontSize: 48, marginBottom: 12 }}>😌</div>
        <h3>Your body is relaxed now. Well done.</h3>
        <button className="btn btn--success" style={{ marginTop: 20 }} onClick={onDone}>✅ I feel relaxed</button>
      </div>
    );
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div style={{ textAlign: 'center' }}>
        <div style={{ fontSize: '2rem', fontWeight: 700, color: phase === 'tense' ? 'var(--color-warning)' : 'var(--color-success)', marginBottom: 4 }}>
          {timer}s
        </div>
      ...
      <div style={{ display: 'flex', gap: 6, justifyContent: 'center' }}>
        {PMR_STEPS.map((_, i) => (
          <div key={i} style={{ width: 8, height: 8, borderRadius: '50%', background: i < step ? 'var(--color-success)' : i === step ? 'var(--color-primary)' : 'var(--color-border)' }} />
        ))}
      </div>
    </div>
  );
...
// ── Main Screen ───────────────────────────────────────────────────────────────
const EXERCISES = [
  { id: 'box', label: 'Box Breathing', emoji: '⬜', desc: '4-4-4-4 calm reset' },
  { id: '478', label: '4-7-8 Breathing', emoji: '🌊', desc: 'Deep sleep technique' },
  { id: 'grounding', label: '5-4-3-2-1 Grounding', emoji: '🌿', desc: 'Sensory anchor technique' },
  { id: 'pmr', label: 'Muscle Relaxation', emoji: '💆', desc: 'Body scan & release' },
];
  ...
  return (
    <div className="screen" style={{ padding: '0 0 16px' }}>
      <div className="page-header">
        <button className="page-header__back" onClick={active ? () => setActive(null) : () => navigate(-1)} aria-label="Back">‹</button>
        <h2 className="page-header__title">{active ? EXERCISES.find((e) => e.id === active)?.label : 'Breathing & Grounding'}</h2>
      </div>

      <div style={{ padding: '16px' }}>
        {!active ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <p style={{ marginBottom: 8 }}>Choose an exercise. All are free and need no account.</p>
            {EXERCISES.map((ex) => (
              <button
                key={ex.id}
              ...
              >
                ...
                <div>
                  <div style={{ fontWeight: 700, fontSize: '0.95rem', color: 'var(--color-text)' }}>{ex.label}</div>
                  <div style={{ fontSize: '0.8rem', color: 'var(--color-text-muted)', marginTop: 2 }}>{ex.desc}</div>
                </div>
              </button>
            ))}
          </div>
          ...
          <div style={{ padding: '8px 0' }}>
            {active === 'box' && <BoxBreathing onDone={handleDone} />}
            {active === '478' && <Breathing478 onDone={handleDone} />}
            {active === 'grounding' && <Grounding54321 onDone={handleDone} />}
            {active === 'pmr' && <PMR onDone={handleDone} />}
          </div>
        )}
      </div>
    </div>
